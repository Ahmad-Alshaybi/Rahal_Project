...
// 3. إعدادات الترقيم (Pagination) - يجب تعريفها هنا قبل استخدامها
$per_page = 9; // عدد المعالم في كل صفحة
$current_page = isset($_GET['page']) ? max(1, intval($_GET['page'])) : 1;
$offset = ($current_page - 1) * $per_page;
$total_attractions = 0;
$total_pages = 1;

// 4. بناء الاستعلام مع الفلاتر
...
$count_sql = "SELECT COUNT(t.id) as total 
...
if ($count_result) {
    ...
    $total_attractions = $row['total'];
}

// حساب عدد الصفحات
$total_pages = ceil($total_attractions / $per_page);
...
// بناء الاستعلام للبيانات مع الترقيم
$sql = "SELECT 
            t.*,
...
// إذا لم يكن هناك معالم مع الفلاتر المحددة، نجلب كل المعالم بدون ترقيم
if (empty($filtered_attractions) && (!empty($selected_region) || !empty($selected_city) || !empty($selected_category))) {
    // إعادة تعيين الترقيم لجلب كل المعالم
    $per_page = 1000; // عدد كبير جداً لجلب كل المعالم
    $current_page = 1;
    $offset = 0;
    ...
    if ($result && $result->num_rows > 0) {
        $filtered_attractions = $result->fetch_all(MYSQLI_ASSOC);
        $total_attractions = count($filtered_attractions);
        $total_pages = 1;
    }
}
?>
...
<div class="page-header" dir="rtl">
    <div class="header-content">
        ...
        <!-- تم إضافة div للـ header-stats هنا -->
        <div class="header-stats">
            <div class="stat-item">
                <span class="stat-number"><?= $total_attractions ?></span>
                <span class="stat-label">إجمالي المعالم</span>
            </div>
            <div class="stat-item">
                <span class="stat-number"><?= count($filtered_attractions) ?></span>
                <span class="stat-label">معلم في هذه الصفحة</span>
            </div>
            <div class="stat-item">
                <span class="stat-number"><?= $current_page ?> / <?= $total_pages ?></span>
    ...
    <div class="attraction-card" style="background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 5px 20px rgba(0,0,0,0.1); transition: transform 0.3s ease; border: 1px solid #eee; text-align: right; direction: rtl; display: flex; flex-direction: column; height: 100%;">
        ...
        <div style="height: 250px; overflow: hidden; flex-shrink: 0;">
            <img loading="lazy" decoding="async" 
                src="<?= htmlspecialchars($attraction['image_url'] ?? '/images/placeholder.jpg') ?>" 
...
<div class="pagination-section" dir="rtl" style="margin: 50px auto; text-align: center; max-width: 1400px;">
    <nav aria-label="Page navigation" style="display: inline-block;">
        ...
        <div class="page-info" style="margin-top: 15px; font-family: 'Tajawal', sans-serif; color: #666; font-size: 14px;">
            ...
            إلى 
            <span style="font-weight: bold; color: #be9d6a;"><?= min($offset + count($filtered_attractions), $total_attractions) ?></span> 
            من أصل 
            <span style="font-weight: bold; color: #142836;"><?= $total_attractions ?></span> 
            معلم
        </div>
    </nav>
</div>
