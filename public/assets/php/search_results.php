...
if (!empty($search_query)) {
    ...
    // تحليل البحث: هل يحتوي على "منطقة"؟
    $is_region_search = false;
    if (strpos($search_query, 'منطقة') !== false) {
        $is_region_search = true;
        // إزالة كلمة "منطقة" للبحث فقط عن اسم المنطقة
        $region_name = str_replace('منطقة', '', $search_query);
        $safe_region_name = $conn->real_escape_string(trim($region_name));
    }
    
    // SQL query بناءً على نوع البحث
    if ($is_region_search && !empty($safe_region_name)) {
        // البحث في المنطقة فقط
        $sql = "SELECT 
                    t.*,
                ...
                WHERE r.name LIKE '%$safe_region_name%'
                ORDER BY t.name";
    } else {
        // البحث في اسم المعلم أو اسم المدينة
        $sql = "SELECT 
                    t.*,
                    r.name AS region_name, 
                    c.name AS city_name 
                ...
                LEFT JOIN cities c ON t.city_id = c.id
                WHERE (
                    -- حالة 1: اسم المعلم يطابق بالضبط أو جزئياً
                    t.name LIKE '%$safe_search_query%'
                ) OR (
                    -- حالة 2: اسم المدينة يطابق بالضبط (لإظهار معالم المدينة فقط)
                    c.name LIKE '%$safe_search_query%'
                )
                ORDER BY 
                    CASE 
                        -- الأولوية: اسم المعلم يطابق بالضبط
                        WHEN t.name = '$safe_search_query' THEN 1
                        -- الثانية: اسم المدينة يطابق بالضبط
                        WHEN c.name = '$safe_search_query' THEN 2
                        -- الثالثة: اسم المعلم يحتوي على الكلمة
                        WHEN t.name LIKE '%$safe_search_query%' THEN 3
                        -- الرابعة: اسم المدينة يحتوي على الكلمة
                        WHEN c.name LIKE '%$safe_search_query%' THEN 4
                        ELSE 5
                    END,
                    t.name";
    }
    ...
    if ($result) {
        if ($result->num_rows > 0) {
            ...
            $per_page = 9; // عدد النتائج في كل صفحة
            ...
            // تقسيم النتائج للصفحة الحالية
            $paged_results = array_slice($search_results, $offset, $per_page);
        }
    } else {
        // في حالة وجود خطأ في الاستعلام
...
<div class="page-header" dir="rtl">
    <div class="header-content">
        ...
        <div class="header-stats">
            ...
            <div class="stat-item">
                <span class="stat-number"><?= count($paged_results) ?></span>
                <span class="stat-label">نتيجة في هذه الصفحة</span>
            </div>
            <div class="stat-item">
                <span class="stat-number"><?= $total_pages > 0 ? $current_page : 0 ?> / <?= $total_pages ?></span>
...
<div class="attractions-grid" dir="rtl">
    <?php if (!empty($paged_results)): ?>
        <?php foreach ($paged_results as $attraction): ?>
    ...
    <?php elseif (!empty($search_query)): ?>
        <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
            <p style="font-size: 18px; color: #666; font-family: 'Tajawal', sans-serif;">
                لم نعثر على أي نتائج لبحثك "<?= htmlspecialchars($search_query) ?>".
            </p>
            <div style="margin-top: 20px;">
                <a href="attractions_list.php" style="display: inline-block; background: #be9d6a; color: white; padding: 12px 30px; text-decoration: none; border-radius: 8px; font-family: 'Tajawal', sans-serif; font-weight: 500; margin: 10px;">
                    استعرض جميع المعالم
                </a>
                <a href="attractions_list.php?city=<?= urlencode($search_query) ?>" style="display: inline-block; background: #142836; color: white; padding: 12px 30px; text-decoration: none; border-radius: 8px; font-family: 'Tajawal', sans-serif; font-weight: 500; margin: 10px;">
                    استعرض معالم <?= htmlspecialchars($search_query) ?>
                </a>
            </div>
        </div>
    <?php endif; ?>
...
<div class="pagination-section" dir="rtl" style="margin: 50px auto; text-align: center; max-width: 1400px;">
    <nav aria-label="Page navigation" style="display: inline-block;">
        <ul class="pagination" style="list-style: none; display: flex; gap: 10px; justify-content: center; align-items: center; flex-wrap: wrap; padding: 0;">
            
            <!-- زر الصفحة الأولى -->
            <?php if ($current_page > 1): ?>
                <li class="page-item">
                    <a class="page-link first-page" 
            ...
            <?php else: ?>
                <li class="page-item disabled">
                    <span class="page-link first-page">
                        <i class="fas fa-angle-double-right"></i>
                    </span>
                </li>
            <?php endif; ?>
            ...
            <?php else: ?>
                <li class="page-item disabled">
                    <span class="page-link last-page">
                        <i class="fas fa-angle-double-left"></i>
                    </span>
                </li>
            <?php endif; ?>
        ...
        <!-- عرض معلومات الصفحة -->
        <div class="page-info" style="margin-top: 15px; font-family: 'Tajawal', sans-serif; color: #666; font-size: 14px;">
            عرض النتائج من 
            <span style="font-weight: bold; color: #be9d6a;"><?= $offset + 1 ?></span> 
            إلى 
            <span style="font-weight: bold; color: #be9d6a;"><?= min($offset + count($paged_results), $total_results) ?></span> 
            من أصل 
            <span style="font-weight: bold; color: #142836;"><?= $total_results ?></span> 
            نتيجة
        </div>
    </nav>
</div>
...
<style>
    ...
    .pagination .page-link:hover {
        ...
        box-shadow: 0 5px 15px rgba(190, 157, 106, 0.1);
    }
    ...
    .pagination .page-item.active .page-link {
        ...
        border-color: #be9d6a;
        font-weight: 600;
        box-shadow: 0 4px 10px rgba(190, 157, 106, 0.3);
    }
